...
def create_comparison_plots(original, iqr_zeroed, zscore_zeroed, output_dir, base_name):
    ...
    plt.plot(original, 'b-', alpha=0.5)
    plt.subplot(2, 2, 2)
    plt.title('IQR Zeroed Data')
    plt.plot(iqr_zeroed, 'g-', alpha=0.7)
    
    # Distribution comparison
    plt.subplot(2, 2, 3)
    plt.hist(original, bins=50, color='blue', alpha=0.7, label='Original')
    plt.hist(iqr_zeroed[iqr_zeroed != 0], bins=50, color='green', alpha=0.7, label='IQR Valid')
    plt.legend()
    
    plt.subplot(2, 2, 4)
    plt.hist(original, bins=50, color='blue', alpha=0.7, label='Original')
    plt.hist(zscore_zeroed[zscore_zeroed != 0], bins=50, color='red', alpha=0.5, label='Z-score Valid')
    plt.legend()
    ...
    plt.plot(zscore_zeroed, 'r-', alpha=0.5, label='Z-score Zeroed')
    plt.legend()
    
    plt.subplot(1, 2, 2)
    plt.title('Zeroed Points Comparison')
    plt.bar(['IQR Zeroed', 'Z-score Zeroed'], 
            [np.sum(iqr_zeroed == 0), np.sum(zscore_zeroed == 0)],
            color=['green', 'red'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--output', default='processed_output', 
                      help='Root output directory (default: processed_output)')
